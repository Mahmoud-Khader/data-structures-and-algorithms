...
const maxInArray = (arr) => {
  // Solution code here...
  const [first] = arr;
  const maxValue = arr.reduce((acc, value) => Math.max(acc, value), first)
  return maxValue;
};
...
const checkValues = (obj, value) => {
  // Solution code here...
  let result;
  let solve = Object.values(obj);
  solve.forEach(item => {
    return (item == value ? result = true : result = false)
  });
  return result;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.map(value => {
    if (value.name == character) {
      value.children ? result = true : result = false;
    }
  });
  return result;
};
